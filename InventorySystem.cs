...
public class InventorySystem : MonoBehaviour
{
    ...
    void Start()
    {
       // message_text.GetComponent<TMP_Text>().text = "Inventory";
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0) && itemIndexToDrag > -1)
        {
            if (hoveringOverIndex < 0)
            {
                ...
                Instantiate(availableItems[itemSlots[itemIndexToDrag]], playerCamera.transform.position + (playerCamera.transform.forward), Quaternion.identity);
    ...
    void FixedUpdate()
    {
        ...
        if((detectedItem == null)&&(itemTmp !=null))
        {
                   
                    // itemTmp = objectHit.GetComponent<PickItem>();
...
/*
    ...
    {
        //Inventory UI
        message_text.GetComponent<TMP_Text>().text = "Press 'Tab' to open Inventory";

        //Inventory window
        if (windowAnimation < 1)
        {
            GUILayout.BeginArea(new Rect(10 - (430 * windowAnimation), Screen.height / 2 - 100, 302, 430/2), customBox);
                ...
                for (int a = 0; a < 3; a++)
                {
                    if (i + a < itemSlots.Length)
                    {
                        ...
                        else
                        {
                            //Empty slot
                           // GUILayout.Box(image1);
                            GUILayout.Box(image1, GUILayout.Width(95), GUILayout.Height(95));
                        }
        ...
        if (!showInventory)
        {
            ...
            if (detectedItem)
            {
                ...
                message_text.GetComponent<TMP_Text>().text = "Press 'F' to pick '";
